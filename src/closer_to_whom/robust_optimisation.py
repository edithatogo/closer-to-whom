...
def robust_p_median_oracle(
    ...
    `scenario_costs` has shape scenario x origin x candidate facility.
    """

    costs = np.asarray(scenario_costs, dtype=np.float64)
    weights = np.asarray(demand, dtype=np.float64)
    if costs.ndim != 3:
        raise ValueError("scenario_costs must be scenario x origin x facility")
    ...
    return sorted(
        rows, key=lambda row: (row.maximum_regret, row.expected_objective, row.selected_indices)
    )
