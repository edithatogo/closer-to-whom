...
def build(output: Path) -> None:
    ...
    optimisation = _read("optimisation-comparison.json")
    rows = []
    ...
    links = "".join(
        ...
        for name, label in (
            ...
            ("optimisation-comparison.json", "Exact optimisation comparison"),
        )
    )
    report_names = (
...
<h2>Evidence and interpretation</h2><ul><li>Optimisation frontier: {html.escape(frontier["status"])}; optimality claimed: {str(frontier["optimality_claimed"]).lower()}.</li><li>Exact and robust optimisation comparisons are bounded to the enumerated candidate set; the robust stress case is hypothetical, not an observed disruption forecast.</li><li>The multi-objective frontier minimises aggregate access, travel-dispersion, candidate-site count, and hypothetical stress delta; it is not a policy recommendation.</li><li>Uncertainty is separated into spatial, temporal-demand, and deterministic cost scenarios; probabilistic intervals are not estimated.</li><li>MCDA and VOI outputs are exploratory research artifacts and do not recommend a policy.</li></ul>
...
<p>MCDA status: {html.escape(str(mcda.get("status", "not declared")))}. VOI status: {html.escape(str(voi.get("status", "not declared")))}. Equity status: {html.escape(str(equity.get("status", "not declared")))}. Capacity/cost status: {html.escape(str(capacity.get("status", "not declared")))}. Resilience status: {html.escape(str(resilience.get("status", "not declared")))}. Exact optimisation status: {html.escape(str(optimisation.get("status", "not declared")))}.</p></main></body></html>\n"""
