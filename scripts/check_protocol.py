...
def main() -> int:
    ...
    if (
        protocol.get("human_participants")
        or protocol.get("confidential_data")
        or protocol.get("microdata")
    ):
        failures.append(
            "current protocol cannot include participants, confidential data, or microdata"
        )
    ...
    if "actual_patient_journey" not in claims.get(
        "forbidden_claim_classes_without_new_evidence", []
    ):
