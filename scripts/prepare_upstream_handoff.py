...
def main() -> int:
    ...
    if isinstance(libraries,dict):
        libraries=[dict(value,id=key) if isinstance(value,dict) else {"id":key,"state":value} for key,value in libraries.items()]
    rows=[]
    for item in libraries if isinstance(libraries,list) else []:
        if not isinstance(item,dict):
            continue
        identifier=str(item.get("id",item.get("name","unknown")))
        issue_candidates=list((ROOT / "upstream" / "issues").glob(f"*{identifier}*")) if (ROOT / "upstream" / "issues").exists() else []
        patch_candidates=list((ROOT / "upstream" / "patches").glob(f"*{identifier}*")) if (ROOT / "upstream" / "patches").exists() else []
        rows.append({
            "library":identifier,
            ...
            "local_compatibility_implementation": item.get("local_compatibility_implementation"),
        })
    output=ROOT / "release" / "upstream-handoff.json"
    output.write_text(json.dumps({"schema_version":"1.0.0","libraries":rows},indent=2,sort_keys=True)+"\n")
    print(output)
    return 0
