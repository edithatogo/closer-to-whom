...
The core rule is **unknown is not absent**. Lack of qualifying public evidence produces `unknown`, never `false`. Grade 1, drug-specific claims require a dated, role-appropriate external evidence receipt before publication. The repository is a sole-developer code harness and does not require a second GitHub approval. A conservative network uses only the strongest qualifying evidence; structural sensitivity analyses progressively include weaker but plausible evidence grades.
