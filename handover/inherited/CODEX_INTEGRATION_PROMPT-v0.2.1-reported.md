...
- `closer-to-whom-v0.3.0-handover.bundle` — preferred; contains Git history and tags.
- `closer-to-whom-v0.3.0-handover.zip` — source snapshot without `.git`.

You may also receive `closer-to-whom-v0.3.0-handover-release-manifest.json`, `closer-to-whom-v0.3.0-handover-SHA256SUMS`, the independently consumable `closer-to-whom-v0.3.0-handover-GITHUB_ECOSYSTEM_EXPORTS.zip`, and standalone ecosystem audit JSON/Markdown files.
...
```bash
sha256sum -c closer-to-whom-v0.3.0-handover-SHA256SUMS

git clone closer-to-whom-v0.3.0-handover.bundle closer-to-whom
cd closer-to-whom
git bundle verify ../closer-to-whom-v0.3.0-handover.bundle
git verify-tag v0.3.0-handover || git show v0.3.0-handover
```

When integrating into an existing repository rather than cloning:

```bash
git remote add handover /absolute/path/closer-to-whom-v0.3.0-handover.bundle
...
```bash
unzip closer-to-whom-v0.3.0-handover.zip
cd closer-to-whom-v0.3.0-handover
git init -b main
git add .
git commit -m 'chore: import closer-to-whom v0.3.0 handover'
```
...
9. `ecosystem/github-repositories.yaml`
10. `ecosystem/pipelines.yaml`
11. `ecosystem/tool-contracts.yaml`
12. `schemas/arrow/index.json`
13. `schemas/json/index.json`
14. `HANDOVER.md`
15. the active track under `conductor/tracks/`
...
## Owner-repository ecosystem and upstream libraries

Review `ecosystem/github-repositories.yaml`, `ecosystem/pipelines.yaml`, `ecosystem/tool-contracts.yaml`, and `upstream/registry.yaml`. The snapshot catalogues all 165 public repositories, identifies 96 public forks, and assigns 137 suitable repositories to bounded pipelines, but none is a mandatory runtime dependency. “Included” does not mean installed, trusted, API-compatible, or redistributable.

Generate a safe core workspace plan with `uv run python scripts/materialize_ecosystem.py --tier core`, or plan all suitable repositories with `uv run python scripts/materialize_ecosystem.py --all-selected`. Cloning requires both `--allow-network` and `--allow-write`; acquired code must never be executed automatically.

For functionality that belongs in any core repository (`fyi-cli`, `fyi-archive`, `open_social_data`, `osf-cli-go`, `reimbursement-atlas`, `rac-conformance`, `innovate`, `voiage`, `gtpcnz`, `healthpoint-rs`, `kairos`, `authentext`, `sourceright`, `mchs`, `mars`, `ginsim`, `conductor-next`, or `vop_poc_nz`):
